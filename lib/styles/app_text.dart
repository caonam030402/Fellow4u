import 'package:flutter/material.dart';

class AppText {
  static const display = TextStyle(fontSize: 48, fontWeight: FontWeight.w500);
  static const heading1 = TextStyle(fontSize: 34, fontWeight: FontWeight.w600);
  static const heading2 = TextStyle(fontSize: 24, fontWeight: FontWeight.w600);
  static const heading3 = TextStyle(fontSize: 18, fontWeight: FontWeight.w600);
  static const body1 = TextStyle(fontSize: 16, fontWeight: FontWeight.w400);
  static const body2 = TextStyle(fontSize: 15, fontWeight: FontWeight.w400);
  static const caption = TextStyle(fontSize: 14, fontWeight: FontWeight.w400);
  static const helperText =
      TextStyle(fontSize: 12, fontWeight: FontWeight.w400);
  static const quote = TextStyle(fontSize: 28, fontWeight: FontWeight.w400);
}
